...
export default function GovernanceCountdown() {
  ...
  return (
    <div className="container mx-auto my-8">
      <h2 className="text-center text-sm xs:text-base sm:text-lg mb-0 xs:mb-1 sm:mb-2 text-foreground-500 font-roboto-mono">Governance Can Begin In</h2>
      <div
      ...
      >
        {Object.entries(timeLeft).map(([key, value]) => (
          <div key={key} className="flex flex-col items-center grow basis-0">
            ...
            <div className="text-xs xs:text-sm sm:text-base uppercase text-foreground-500 font-roboto-mono">{key}</div>
          </div>
        ))}
      </div>
    </div>
